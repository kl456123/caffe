...
namespace caffe
{
                    ...
                    Blob<Dtype> *out)
    {
        ...
        int o_head, k_head;
        //position sensetive group
        bool position_sensetive = conv_param->position_sensetive();
        int ps_group = 1;
        if(position_sensetive)
        {
            if(conv_param->has_ps_group())
            {
                ps_group = conv_param->ps_group();
            }
            else
            {
                ps_group = kernel_w * kernel_h;
            }
        }
        int i_ps_g = k_g / ps_group;
                                        ...
                                        for (int p = 0; p < kernel_h; p++)
                                        {
                                            for (int q = 0; q < kernel_w; q++)
                                            {
                                                        ...
                                                        && in_x >= 0 && in_x < in->shape(3 + has_depth))
                                                {
                                                    ...
                                                    in_offset[0] = n;
                                                    int ps_g_idx = p * kernel_w + q;
                                                    if(!position_sensetive)
                                                    {
                                                        ps_g_idx = 0;
                                                    }
                                                    in_offset[1] = k_head + k + i_ps_g * ps_g_idx;
                                                    if (has_depth)
                                                    {
                                                        in_offset[2] = in_z;
                                                    }
    ...
    TYPED_TEST(ConvolutionLayerTest, TestPositionSensetiveConvolution)
    {
        typedef typename TypeParam::Dtype Dtype;
        LayerParameter layer_param;
        ConvolutionParameter *convolution_param =
        ...
        convolution_param->set_num_output(4);
        convolution_param->set_ps_group(9);
        convolution_param->set_position_sensetive(true);
        // this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
        // this->blob_top_vec_.push_back(this->blob_top_2_);
        //reshape blob_bottom
        vector<int> blob_bottom_shape(this->blob_bottom_->shape());
        blob_bottom_shape[2] = 8;
        blob_bottom_shape[3] = 8;
        blob_bottom_shape[1] = 90;
        this->blob_bottom_->Reshape(blob_bottom_shape);
        //refill data into bottom blobs
        this->Refill();
        convolution_param->mutable_weight_filler()->set_type("gaussian");
        // convolution_param->mutable_weight_filler()->set_value(2);
    ...
    TYPED_TEST(ConvolutionLayerTest, TestPositionSensetiveConvolutionGradient)
    {
        typedef typename TypeParam::Dtype Dtype;
        LayerParameter layer_param;
        //reshape bottom blobs for adaption here
        vector<int> blob_bottom_shape(this->blob_bottom_->shape());
        blob_bottom_shape[1] = 90;
        blob_bottom_shape[2] = 8;
        blob_bottom_shape[3] = 8;
