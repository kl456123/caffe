...
namespace caffe {
    template <typename Dtype>
        __global__ void im2col_gpu_ps_kernel(const int n, const Dtype* data_im,
            ...
            CUDA_KERNEL_LOOP(index, n) {
                ...
                const Dtype* data_im_ptr=NULL ;
                for (int i = 0; i < kernel_h; ++i) {
                    for (int j = 0; j < kernel_w; ++j) {
                        int group_index = kernel_w*i+j;
                        data_im_ptr = data_im +  ((c_im+group_index*channels) * height + h_offset)\
                                      * width + w_offset;
    ...
    template <typename Dtype>
        void im2col_gpu_ps(const Dtype* data_im, const int channels,
                const int height, const int width, const int kernel_h, const int kernel_w,
                const int pad_h, const int pad_w,
                const int stride_h, const int stride_w,
                const int dilation_h, const int dilation_w,
            ...
            //note that channels here refers to conv_in_channels/ps_group
            ...
            im2col_gpu_ps_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
                CAFFE_CUDA_NUM_THREADS>>>(
                        num_kernels, data_im, height, width, kernel_h, kernel_w, pad_h,
                        pad_w, stride_h, stride_w, dilation_h, dilation_w, height_col,
                        width_col, data_col,channels);
            CUDA_POST_KERNEL_CHECK;
        }

    // Explicit instantiation
    template void im2col_gpu_ps<float>(const float* data_im, const int channels,
            const int height, const int width, const int kernel_h, const int kernel_w,
            const int pad_h, const int pad_w, const int stride_h, const int stride_w,
            const int dilation_h, const int dilation_w, float* data_col);
    template void im2col_gpu_ps<double>(const double* data_im, const int channels,
            const int height, const int width, const int kernel_h, const int kernel_w,
            const int pad_h, const int pad_w, const int stride_h, const int stride_w,
            const int dilation_h, const int dilation_w, double* data_col);

    template <typename Dtype>
        __global__ void col2im_gpu_ps_kernel(const int n, const Dtype* data_col,
                ...
                const int height_col, const int width_col,
                Dtype* data_im) {
            /*         for(int index=0;index<height*width*channels*ps_group;index++){ */

            /* data_im[index] = Dtype(0); */
            /* } */
            CUDA_KERNEL_LOOP(index, n) {
                const int h_index = index / width_col;
                const int h_col = h_index % height_col;
                ...
                for (int i = 0; i < kernel_h; ++i) {
                    for (int j = 0; j < kernel_w; ++j) {
                        ...
                        int h_im = h_offset + i * dilation_h;
                        int w_im = w_offset + j * dilation_w;
                        int index = i * dilation_h * width + j * dilation_w;
                        data_im_ptr[index] +=
                            (h_im >= 0 && w_im >= 0 && h_im < height && w_im < width) ?
                            *data_col_ptr: 0;
                        data_col_ptr += height_col * width_col;
    ...
    template <typename Dtype>
        void col2im_gpu_ps(const Dtype* data_col, const int channels,
                const int height, const int width, const int kernel_h, const int kernel_w,
                const int pad_h, const int pad_w, const int stride_h,
                const int stride_w, const int dilation_h, const int dilation_w,
                Dtype* data_im,const int ps_group) {
            ...
            // NOLINT_NEXT_LINE(whitespace/operators)
            // caffe_set(height * width * channels*ps_group , Dtype(0), data_im);
            const int N= height*width*channels*ps_group;
            /* cudaMemset(data_im,Dtype(0),N*sizeof(Dtype)); */
            caffe_gpu_set(N,Dtype(0),data_im);
            col2im_gpu_ps_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
                CAFFE_CUDA_NUM_THREADS>>>(
    ...
    // Explicit instantiation
    template void col2im_gpu_ps<float>(const float* data_col, const int channels,
            const int height, const int width, const int kernel_h, const int kernel_w,
            const int pad_h, const int pad_w, const int stride_h,
            const int stride_w, const int dilation_h, const int dilation_w,
            float* data_im, const int ps_group);
    template void col2im_gpu_ps<double>(const double* data_col, const int channels,
            const int height, const int width, const int kernel_h, const int kernel_w,
            const int pad_h, const int pad_w, const int stride_h,
            const int stride_w, const int dilation_h, const int dilation_w,
            double* data_im, const int ps_group);
